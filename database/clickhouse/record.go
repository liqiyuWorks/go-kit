...
func InsertRecord(engineName string, tableName string, objPtr interface{}) (affectedRows int64, err error) {
	...
	affectedRows = 0
	err = nil
	result := GCkManager.EngineMap[engineName].Table(tableName).Create(objPtr)
	if result.Error != nil {
		base.Glog.Errorf("%s: %v", statuscode.ERROR_CK_INSERT_RECORD.Msg, result.Error)
...
func FindRecordsByCond(engineName string, tableName string, cond string, page int, pageSize int, resultSlicePtr interface{}) error {
	...
	if result := GCkManager.EngineMap[engineName].Table(tableName).Where(cond).Find(resultSlicePtr); result.Error != nil {
		base.Glog.Errorf("%s: %v", statuscode.ERROR_CK_FIND_RECORDS.Msg, result.Error)
		return result.Error
	}
	return nil
}
...
func UpdateRecordByCond(engineName string, tableName string, cond string, fieldMap map[string]interface{}) (affectedRows int64, err error) {
	...
	affectedRows = 0
	err = nil
	if result := GCkManager.EngineMap[engineName].Table(tableName).Where(cond).Updates(fieldMap); result.Error != nil {
		base.Glog.Errorf("%s: %v", statuscode.ERROR_CK_UPDATE_RECORD.Msg, result.Error)
		return result.RowsAffected, result.Error
	}
	return
}
...
func DeleteRecordByCond(engineName string, tableName string, cond string, objPtr interface{}) (affectedRows int64, err error) {
	...
	affectedRows = 0
	err = nil
	if result := GCkManager.EngineMap[engineName].Table(tableName).Where(cond).Delete(objPtr); result.Error != nil {
		base.Glog.Errorf("%s: %v", statuscode.ERROR_CK_DELETE_RECORD.Msg, result.Error)
		return result.RowsAffected, result.Error
	}
	return
}
